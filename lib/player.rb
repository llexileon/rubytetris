class PlayerInput
  def initialize(window, shapelogic)
    @window = window
  ...
  def button_down(id)
    ...
    when Gosu::Button::KbZ
      @shapelogic.rotate_counter_clockwise
    when Gosu::Button::KbX
      @shapelogic.rotate_clockwise
    end
  end
  ...
  def update
    ...
    if @das_counter == 0 || @das_counter > @das_delay && (@das_counter - @das_delay) % @das_repeat == 0
      ...
      @shapelogic.move_right if @das_direction == :right
      @shapelogic.move_left  if @das_direction == :left
    end
    
    if input_down?
      @shapelogic.move_down if @soft_drop_counter % @soft_drop_repeat == 0
      @soft_drop_counter += 1
    else
      @soft_drop_counter = 0
    end
